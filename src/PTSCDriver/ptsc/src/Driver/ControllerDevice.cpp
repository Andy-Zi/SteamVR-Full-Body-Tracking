...
ptscDriver::ControllerDevice::ControllerDevice(std::string serial, ControllerDevice::Handedness handedness):
    serial_(serial),
    handedness_(handedness)
{
}

std::string ptscDriver::ControllerDevice::GetSerial()
{
    return this->serial_;
}

void ptscDriver::ControllerDevice::Update()
{
...
DeviceType ptscDriver::ControllerDevice::GetDeviceType()
{
    return DeviceType::CONTROLLER;
}

ptscDriver::ControllerDevice::Handedness ptscDriver::ControllerDevice::GetHandedness()
{
    return this->handedness_;
}

vr::TrackedDeviceIndex_t ptscDriver::ControllerDevice::GetDeviceIndex()
{
    return this->device_index_;
}

vr::EVRInitError ptscDriver::ControllerDevice::Activate(uint32_t unObjectId)
{
    ...
    GetDriver()->GetProperties()->SetStringProperty(props, vr::Prop_ModelNumber_String, "eptsc_controller");

    // Set up a render model path
    GetDriver()->GetProperties()->SetStringProperty(props, vr::Prop_RenderModelName_String, "{ptsc}ptsc_controller");
    ...
    GetDriver()->GetProperties()->SetStringProperty(props, vr::Prop_InputProfilePath_String, "{ptsc}/input/ptsc_controller_bindings.json");
    ...
    std::string controller_ready_file = "{ptsc}/icons/controller_ready_" + controller_handedness_str + ".png";
    std::string controller_not_ready_file = "{ptsc}/icons/controller_not_ready_" + controller_handedness_str + ".png";
...
void ptscDriver::ControllerDevice::Deactivate()
{
    this->device_index_ = vr::k_unTrackedDeviceIndexInvalid;
}

void ptscDriver::ControllerDevice::EnterStandby()
{
}

void* ptscDriver::ControllerDevice::GetComponent(const char* pchComponentNameAndVersion)
{
    return nullptr;
}

void ptscDriver::ControllerDevice::DebugRequest(const char* pchRequest, char* pchResponseBuffer, uint32_t unResponseBufferSize)
{
    if (unResponseBufferSize >= 1)
        pchResponseBuffer[0] = 0;
}

vr::DriverPose_t ptscDriver::ControllerDevice::GetPose()
{
    return last_pose_;
}
